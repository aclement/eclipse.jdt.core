...
private void decodeTypeAnnotations(int offset, boolean runtimeVisible) {
	...
	if (numberOfAnnotations > 0) {
		...
		if (this.annotations == null) {
			if (newInfoCount != newInfos.length)
				System.arraycopy(newInfos, 0, newInfos = new TypeAnnotationInfo[newInfoCount], 0, newInfoCount);
			this.typeAnnotations = newInfos;
		} else {
			int length = this.typeAnnotations.length;
			TypeAnnotationInfo[] temp = new TypeAnnotationInfo[length + newInfoCount];
